/* ============================================================
   ...
   Chargé par le layout automatismes-2nde.
   Projette les deux panneaux : Séance classique + Devoir.
   ============================================================ */
(function () {
  'use strict';

  var btn = document.getElementById('btn-project');
  var panelSeance = document.getElementById('panel-seance');
  var panelDevoir = document.getElementById('panel-devoir');
  if (!btn || (!panelSeance && !panelDevoir)) return;

  /* ── Lecture des données par panneau ── */
  function readPanel(panel) {
    if (!panel) return [];
    return [].slice.call(panel.querySelectorAll('.auto1re-card')).map(function (card) {
      return {
        numEl  : card.querySelector('.auto1re-num'),
        qNode  : card.querySelector('.auto1re-enonce'),
        corrDiv: card.querySelector('.auto1re-corr > div')
      };
    });
  }

  var panels = [
    { titre: 'Séance', slug: 'S', data: readPanel(panelSeance) },
    { titre: 'Devoir', slug: 'D', data: readPanel(panelDevoir) }
  ...
  /* ── Ouverture / fermeture ── */
  function open() {
    /* Démarre sur le panneau actif si possible */
