...
La droite $(h)$ se nomme aussi (AC), (CA), (AE), (EA), (CE) ou (EC).
\item Citer trois points alignés.\\
 Les points A, C et E car ils appartiennent à la même droite.
\item Donner deux droites perpendiculaires.\\
 $(\text{BD}) \perp (d)$ ou $(\text{BD}) \perp (d')$
\item Donner deux droites parallèles : $(d) \mathbin{/\!/} (d')$
\item Donner deux droites sécantes mais non perpendiculaires et préciser leur point d'intersection.\\
 Les droites $(d)$ et $(h)$ sont sécantes en A et non perpendiculaires.
\end{enumerate}
\end{minipage}%
\hfill
% --- FIGURE ---
