...
\begin{tikzpicture}[scale=1]
  ...
  % Longueurs sur les côtés
  \node[below]        at ($(M)!0.5!(A)$) {5 cm};
  \node[left]         at ($(M)!0.5!(T)$) {4 cm};
  \node[right]        at ($(A)!0.5!(T)$) {6 cm};
\end{tikzpicture}
\end{center}
\end{Méthode}
  \vspace{0.3cm}
...
\begin{tikzpicture}[scale=1]
  ...
  % Longueurs
  \node[below]  at ($(T)!0.5!(R)$)        {5 cm};
  \node[left]   at ($(T)!0.5!(S)$)        {3 cm};
...
\begin{tikzpicture}[scale=1]
  ...
  % Angle en F (40°)
  \draw ($(F) + (-1, 0)$) arc (180:140:1);
  \node at ($(F) + (-1.3, 0.30)$) {$40$°};
\end{tikzpicture}
\end{center}
\end{Méthode}
...
On sait que $\widehat{ABC} + \widehat{BCA} + \widehat{CAB} = 180$°.\\
Puisque $\widehat{BAC} + \widehat{ABC} = 78$°$ + 60$°$ = 138$°, alors $\widehat{ACB} = 180$°$ - 138$°$ = 42$°.
...
\begin{tikzpicture}[scale=0.8]
  ...
  \coordinate (R) at (5, 0);
  \coordinate (S) at (1.5, 2.598);
  \coordinate (O) at (2.5, 0.289);

  % Médiatrices (construction)
  \draw[dashed, gray] (2.5, -0.5) -- (2.5, 3.2);
  \draw[dashed, gray] (0, 1.732) -- (4.5, -0.866);
...
$DEF$ est un triangle rectangle en $F$. L'angle $\widehat{DFE}$ est un angle droit, l'hypoténuse est le segment $[DE]$. \\
On sait alors que $\widehat{FDE} + \widehat{DEF} = 90$° c'est-à-dire $30$°$ + \widehat{DEF} = 90$°. \\
Par conséquent, $\widehat{DEF} = 90$°$ - 30$°$ = 60$°.
\end{minipage}\hfill  \begin{minipage}[c]{0.35\textwidth}
\begin{center}
\begin{tikzpicture}[scale=1]
...
\begin{Exemple}
\begin{minipage}[c]{0.6\textwidth}
$ABC$ est isocèle en $C$ (le sommet principal), sa base est le côté $[AB]$.\\
Les angles $\widehat{CAB}$ et $\widehat{ABC}$ ont la même mesure.
\end{minipage}\hfill  \begin{minipage}[c]{0.35\textwidth}
