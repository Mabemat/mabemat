...
\item $f(x)=x+1$ \quad\quad $a=1$ et $b=1$
\item $g(x)=2$ \quad\quad\quad\quad $a=0$ et $b=2$
\item $h(x)=-3x-2$ \quad\quad $a=-3$ et $b=-2$
\item $k(x)=\dfrac{3}{4}x-3$ \quad\quad $a=\dfrac{3}{4}$ et $b=-3$
\end{itemize}
\end{minipage} \hfill
\begin{minipage}[c]{0.5\linewidth}
\begin{center}
...
\begin{itemize}[label=\textcolor{Couleur10}{$\bullet$}]
\item La fonction $f$ définie par $f(x)=3x+2$ est croissante car $ a=3>0 $.
\item La fonction $f$ définie par $f(x)=-2x+3$ est décroissante car $ a=-2<0 $.
\item La fonction $f$ définie par $f(x)=5$ est constante car $ a=0 $.
\end{itemize}
\end{Exemples}
...
\begin{tikzpicture}
\tkzTabInit[espcl=1.5,lgt=4]{$x$ / 1 ,variations /1 ,signe de $f(x)=2x+4$ /2 } { $-\infty$ , $-2$ , $+\infty$ };
\tkzTabVar{-/ , R/ , +/};
\tkzTabIma{1}{3}{2}{$0$};
\tkzTabLine{, \color{Couleur8}{-}, z, \color{Couleur3}{+}, };
\end{tikzpicture} 
...
\begin{minipage}[c]{0.5\linewidth}
\begin{tikzpicture}
\tkzTabInit[espcl=1.5,lgt=4]{$x$ / 1 ,variations /1 ,signe de $g(x)=-x+3$ /2 } { $-\infty$ , $3$ , $+\infty$ };
\tkzTabVar{+/ , R/ , -/};
\tkzTabIma{1}{3}{2}{$0$};
\tkzTabLine{, \color{Couleur3}{+}, z, \color{Couleur8}{-}, };
\end{tikzpicture} 
...
\begin{multicols}{2}
\item Pour $(d')$ : \\
Le coefficient directeur est $\color{Couleur3}{a=\dfrac{+2}{+1}=2}$. \\
L'ordonnée à l'origine est $\color{violet}{+2}$. \\
La fonction $f$ représentée par la droite $(d')$ est \\définie par $f(x) = {\color{Couleur3}{2}}x + \color{violet}{2}$. \\
\item Pour $(d)$ : \\
Le coefficient directeur est $\color{Couleur11}{a=\dfrac{-1}{+2}=-0,5}$. \\
L'ordonnée à l'origine est $\color{Couleur10}{-1}$. \\
La fonction $g$ représentée par la droite $(d)$ est \\définie par $g(x) = {\color{Couleur11}{-0,5}}x \color{Couleur10}{-1}$. \\
\end{multicols}
\end{itemize}
\vspace{-0.8cm}
\end{Méthodes}
...
Donc $f(x)=\dfrac{-3}{5} x+b$. \\
Comme $A$ est un point de la droite, on a : $f (-2) = 4$, donc : \\
...
\leavevmode\vspace{-\baselineskip}
\begin{enumerate}[itemsep=1.75em]
	\item \begin{minipage}[t]{\linewidth}On donne le tableau de signes d'une fonction affine  $f$  définie sur $\mathbb R$ :\\\begin{tikzpicture}[baseline, scale=0.75]
