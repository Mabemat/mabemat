...
Avec les données de l'énoncé, $\vec u \begin{pmatrix}1\\2\end{pmatrix}$, on en déduit donc que : $-b = 1$ (soit $b=-1$) et $a=2$. \\L'équation cartésienne est donc de la forme : $ 2 x - y + c=0$. \\Comme $A\in (d)$, ses coordonnées $(5\,;\,-1)$ vérifient l'équation de la droite $(d)$.  \\$\begin{aligned}
       2 \times 5 -1 \times (-1)+ c&=0\\
        10 +1 + c&=0\\
        c&= -11
        \end{aligned}$ \\Une équation cartésienne de la droite $(d)$ est donc de la forme : $\boldsymbol{2x-y-11=0}$.
...
3 \\
2
\end{pmatrix}$. \\Une autre équation cartésienne de $d$ est, par exemple, $-2(2x-3y+1)=0$ c'est à dire $-4x+6y-2=0$.\\ On trouve donc que $\overrightarrow{v}\begin{pmatrix}
-6 \\
-4
\end{pmatrix}=-2\overrightarrow{u}$ est aussi un vecteur directeur de la droite $d$.
\end{Exemple}
...
\draw[Couleur6] (0.5,4.3) node {$x_B-x_A$};
\draw[Couleur6] (3.5,2.5) node {$y_B-y_A$};
...
On commence par calculer le coefficient directeur $m$ :\\ $m=\dfrac{y_B-y_A}{x_B-x_A}=\dfrac{-5-4}{7-3}=\dfrac{-9}{4}=-\dfrac{9}{4}$. \\

L'équation de la droite $(AB)$ est donc de la forme : $y=-\dfrac{9}{4} \times x+p$\\

Comme $A \in (AB)$, les coordonnées du point $A$ vérifient l'équation, donc :\\
$4=-\dfrac{9}{4} \times 3 +p\iff p=4+\dfrac{9}{4} \times 3\iff p=\dfrac{43}{4}$\\
...
Soit le système $\left\{
    \begin{array}{ll}
     3x-2y=6    & \\
       -4x+5y=-1 & 
    \end{array}
\right.$. \\Puisque $3\times {\color{Couleur10}{4}}-2\times {\color{Couleur6}{3}} =6 $ et $-4\times {\color{Couleur10}{4}}+5\times {\color{Couleur6}{3}}=-1$, le couple $({\color{Couleur10}{4}};{\color{Couleur6}{3}})$ est une solution de ce système.

\end{Exemple}

\begin{Méthode}Par subsitution. \\
Résoudre le système 
$(S)\left\{
    \begin{array}{ll}
     3x-y=5    & \\
    ...
    \end{array}
\right.$\\
 La première équation permet d'exprimer $y$ en fonction de $x$. \\Le système $(S)$ est donc équivalent à $\left\{
    \begin{array}{ll}
     {\color{Couleur10}{y=3x-5}}    & \\
       2x+3y=7 & 
    \end{array}
\right.$.\\
...
$\left\{
    \begin{array}{ll}
     {\color{Couleur10}{y=3x-5}}    & \\
       2x+3( {\color{Couleur10}{3x-5}} )=7 & 
    \end{array}
\right.$. On résout la deuxième équation où ne figure que l'inconnue $x$ : $2x+3( {\color{Couleur10}{3x-5}} )=7$ équivaut à $2x+9x-15=7$, soit à $11x=22$, c'est à dire à $x=2$.\\
On remplace $x$ par sa valeur dans l'expression de $y$. On a donc $y=3\times 2-5=1$.\\
\underline{Vérification} : $3\times 2-1=6-1=5$ et $2\times 2+3\times 1=4+3=7$ \\
...
$(S)\left\{
    ...
    \end{array}
\right.$ \\
On multiplie la première équation par ${\color{Couleur10}{2}}$ et la seconde par ${\color{Couleur6}{5}}$.\\
Le système $(S)$ équivaut à $(S)\left\{
    \begin{array}{ll}
     4x-10y=2  & \\
       15x+10y=55 & 
    \end{array}
\right.$. \\On additionne les deux équations membre à membre pour éliminer $y$.\\
On obtient $4x-10y+(15x+10y)=2+55$, soit $19x=57$, c'est à dire $x=3$.\\
On remplace $x$ par sa valeur dans le première équation. On obtient $2\times 3-5y=1$, c'est à dire $6-1=5y$, soit $y=1$.\\
\underline{Vérification} : $2\times 3 -5\times 1=6-5=1$ et $3\times 3 +2\times 1=9+2=11$\\
Le système $(S)$ admet donc une unique solution le couple $(3;1)$.\\
\begin{Remarque}
...
\\
Alors $\overrightarrow{u}\begin{pmatrix}
2\\
4
...
2 & -6\\
4 & -12
\end{vmatrix}=2\times (-12)-4\times(-6)=-24+24=0$. \\
...
\begin{center}
\includegraphics[scale=.5]{IMG/intersection_chap_11}
\end{center}
...
\leavevmode\vspace{-\baselineskip}
	...
	\item \begin{minipage}[t]{\linewidth} Donner par lecture graphique une équation réduite de chacune des droites ci-dessous.
