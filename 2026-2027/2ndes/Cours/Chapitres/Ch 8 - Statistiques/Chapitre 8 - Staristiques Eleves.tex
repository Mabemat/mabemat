...
Calculer la médiane pour chaque série de notes de Jérôme, de Bertrand et de Julie. \vspace{0.3cm}
...
\begin{multicols}{3}
\begin{itemize}[label=\textbullet]
\item  $\text{E}_{(\text{Jérôme})} =$ \ligne
\item $\text{E}_{(\text{Bertrand})} =$ \ligne
\item $\text{E}_{(\text{Julie})} =$ \ligne
\end{itemize}
\end{multicols}
