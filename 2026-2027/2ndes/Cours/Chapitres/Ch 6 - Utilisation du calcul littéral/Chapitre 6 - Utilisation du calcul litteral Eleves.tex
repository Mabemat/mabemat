...
\begin{itemize}[label=\textcolor{Couleur8}{$\bullet$}]
\item $(x+1)(2x+5)=$ \ligne
\item $(2y-3)(3y+5)=$ \ligne
\item $(3z-7)(2z-4)=$ \ligne
\item $(5t^2-2t)(-3t-4)=$ \ligne
\end{itemize}
...
\item $B = 3(2 + 3x) - (5 + 2x)(2 + 3x)$ 
\item $C=(2-5x)^2 -(2-5x)(1+x)$
\item $D = 5(1 - 2x) - (4 + 3x)(2x - 1)$
\item $E=3x^2 -x$
\end{itemize}
\end{multicols}
...
\begin{Propriété}
Pour tout $x$ qui n'annule pas l'expression $Q(x)$, l'équation-quotient $\dfrac{P(x)}{Q(x)}= 0$ équivaut à \\
$P(x) = 0$.
...
Résoudre dans $\R$ les équations suivantes :
\begin{enumerate}
\item $\dfrac{x+2}{x+3} = 0 $ \ligne

\ligne

\ligne

\item $\dfrac{3x+5}{x-1}=0$ \ligne

\ligne

\ligne

\item $\dfrac{(2x+1)(x-3)}{x-4}=0$ \ligne
